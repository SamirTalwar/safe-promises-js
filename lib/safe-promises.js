...
function timeOutAfter(timeout, errorConstructor = (timeout) => new Error(`Timed out after ${timeout} milliseconds.`)) {
    return {
        failWith: function(onFailure) {
            return safePromiseClass(timeout, errorConstructor, onFailure);
        }
    };
}

function safePromiseClass(timeout, timeoutError, onFailure) {
    class SafePromise {
